...
#define CAN_SW_PATCH_VERSION_IRQ_C               3
...
)
{
    ...
    if ( CAN_STARTED != Can_ControllerStatuses[u8ctrId].ControllerState )
    {
        do
        {
    ...
    {
        /* @violates @ref Can_Irq_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        if ((0U == (CAN_CONTROLLERCONFIG_TXPOL_EN_U32 & CanStatic_pControlerDescriptors[u8ctrId].u32Options)))
        {
            Can_IPW_ProcessTx(u8ctrId, u8IdMin, u8IdMax);
        }
        /* @violates @ref Can_Irq_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        if ((0U == (CAN_CONTROLLERCONFIG_RXPOL_EN_U32 & CanStatic_pControlerDescriptors[u8ctrId].u32Options)))
        {
    ...
    else
    {
        ...
        if (0U != can_status)
        {
            if(0U != can_mask)
            {
                ...
                if ( CAN_OK == (Can_ReturnType)Can_IPW_SetControllerMode(u8ctrId,  &(Can_pCurrentConfig->ControlerDescriptors[u8ctrId]), CAN_T_STOP,(boolean)FALSE) )
                {
